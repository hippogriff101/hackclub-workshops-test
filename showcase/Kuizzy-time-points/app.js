...
io.on('connection', (socket) => {
    ...
    socket.once("start", async () => {
        for (const question of questions) {
            await new Promise(async (resolve) => {
                ...
                eventEmitter.emit("start", [new Date(), question.time * 1000])

                setTimeout(() => {
                    eventEmitter.emit("timeUp", question.correctAnswer)
    ...
    socket.on("answer", answer => {
        attempt = [answer, Math.floor(getTimeLeft() / 10)]
    })

    eventEmitter.on("timeUp", (correctAnswer) => {
        if (attempt) {
            if (attempt[0] === correctAnswer) {
                userPointsMap[socket.id][1] += attempt[1]
                socket.emit("correct")
            } else {
